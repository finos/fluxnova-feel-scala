...
Create a sub-class of `org.flowave.feel.FeelEngineClock`. Implement the method `getCurrentTime()` to return the current time. 
...
In the second case, create a new file `org.flowave.feel.FeelEngineClock` in the folder `META-INF/services/`. It must contain the full qualified name of the class.

```
org.flowave.feel.example.MyClock
```
